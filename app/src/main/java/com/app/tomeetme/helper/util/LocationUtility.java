...
public class LocationUtility {
    ...
    public static String getDistanceString(double distance, boolean useMetricSystem) {
        ...
        if (useMetricSystem) {
            if (distance < 1000.0d) {
                result = String.format(Locale.US, "%d " + ToMeetMeApplication.getContext().getString(R.string.unit_meter), (int) distance);
            } else if (distance < 5000.0d) {
                result = String.format(Locale.US, "%.1f " + ToMeetMeApplication.getContext().getString(R.string.unit_kilometer), distance / 1000);
            } else {
                result = String.format(Locale.US, "%d " + ToMeetMeApplication.getContext().getString(R.string.unit_kilometer), (int) distance / 1000);
            }
        } else {
            double distanceMiles = distance * 0.000621371192;

            if (distanceMiles < 0.1d) {
                result = String.format(Locale.US, "%.2f " + ToMeetMeApplication.getContext().getString(R.string.unit_mile), distanceMiles);
            } else if (distanceMiles < 10.0d) {
                result = String.format(Locale.US, "%.1f " + ToMeetMeApplication.getContext().getString(R.string.unit_mile), distanceMiles);
            } else {
                result = String.format(Locale.US, "%d " + ToMeetMeApplication.getContext().getString(R.string.unit_mile), (int) distanceMiles);
            }
        }
